...
void RTreeInitNode(RTREENODE *node)
{
	...
	node->level = -1;
	{
		int n1 = sizeof(RTREEBRANCH);
		int n = (int)((PAGE_SIZE - (2 * sizeof(int))) / sizeof(RTREEBRANCH));
	}
	for (i = 0; i < MAXCARD; i++)
...
int RTreePickBranch(RTREEMBR *rc, RTREENODE *node)
{
	...
	for (i = 0; i<MAXKIDS(node); i++)
	{
		if (node->branch[i].child)
		{
			...
			if (increase < bestIncr || first_time) //增加量最小
			{
				best = i;
				bestArea = area;
				bestIncr = increase;
			...
			else if (increase == bestIncr && area < bestArea)//增加量相等，体积最小
			{
				best = i;
				bestArea = area;
				bestIncr = increase;
